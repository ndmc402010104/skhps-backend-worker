#!/usr/bin/env node
/*
同步 prod 全部資料到 local-dev - 使用原始數據（約束已修復）
*/
...
async function main() {
  ...
  try {
    console.log('\n🔄 開始同步 prod 全部資料到 local-dev（約束已修復）\n');
    ...
    // 3. 複製到 local-dev（只改 ID，保持所有原始數據）
    console.log('⏳ 複製到 local-dev（使用原始數據）...');
    let successCount = 0;
    let errorCount = 0;
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      const localRecord = {
        ...record,
        id: generateUUID(),  // 只改 ID
        env: 'local-dev'
        // 保持所有其他字段完全相同：name, employee_id, status, source 等
      };
