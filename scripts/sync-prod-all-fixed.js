#!/usr/bin/env node
/*
同步 prod 全部資料到 local-dev - 改 name 加後綴避免約束
*/
...
async function main() {
  ...
  try {
    console.log('\n🔄 開始同步 prod 全部資料到 local-dev（改name避免約束）\n');
    ...
    // 3. 複製到 local-dev（改 name 加後綴避免約束）
    console.log('⏳ 複製到 local-dev（改name加亂數後綴）...');
    let successCount = 0;
    let errorCount = 0;

    for (let i = 0; i < records.length; i++) {
      const record = records[i];
      // 生成唯一的後綴 (前 8 個 UUID 字符)
      const suffix = generateUUID().substring(0, 8);
      
      const localRecord = {
        ...record,
        id: generateUUID(),
        name: `${record.name}_${suffix}`,  // name 加後綴確保唯一性
        env: 'local-dev'
