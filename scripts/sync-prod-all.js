...
async function main() {
  ...
  try {
    console.log('\n🔄 開始同步 prod 全部資料到 local-dev（只改ID）\n');
    ...
    // 3. 複製到 local-dev（只改ID）
    console.log('⏳ 複製到 local-dev（只改ID）...');
    let successCount = 0;
    let errorCount = 0;
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      const localRecord = {
        ...record,
        id: generateUUID(),  // 新 UUID
        env: 'local-dev'
        // 保持所有其他字段不變，包括 status, source 等等
      };
