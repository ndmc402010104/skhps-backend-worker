#!/usr/bin/env node
/*
同步 prod 全部資料到 local-dev - 改 employee_id 為 NULL
*/
...
async function main() {
  ...
  try {
    console.log('\n🔄 開始同步 prod 全部資料到 local-dev（改 employee_id 為 NULL）\n');
    ...
    // 3. 複製到 local-dev（改 employee_id 為 NULL，改 name 加後綴）
    console.log('⏳ 複製到 local-dev（改 employee_id → NULL）...');
    let successCount = 0;
    let errorCount = 0;
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      const localRecord = {
        ...record,
        id: generateUUID(),
        employee_id: null,  // 改為 NULL 避免約束
        name: `${record.name}_${suffix}`,
        env: 'local-dev'
        // 保持 status, source 等不變
      };
